...
function dal_fetch_events_after($target_id, $target_role, $last_event_id, $limit)
{
    ...
    $mysqli_result = @mysqli_query($connection, "SELECT max(ev.id) AS id, concat('[', group_concat(ev.event_list), ']') as ev_list FROM (SELECT max(id) as id, concat('{\"', type, '\":[', group_concat(DISTINCT concat('\"', message, '\"')), ']}') AS event_list FROM (SELECT type, message, id FROM " . get_event_db_name() . ".event WHERE id > $last_event_id AND (target_id = $target_id OR target_id is null) AND target_role=$target_role LIMIT $limit) AS t_event GROUP BY t_event.type) AS ev");
...
function dal_last_event_id()
{
    ...
    $mysqli_result = mysqli_query($connection, "SELECT max(id) AS id FROM " . get_event_db_name() . ".event;");
    $result = mysqli_fetch_array($mysqli_result, MYSQLI_ASSOC);
    mysqli_free_result($mysqli_result);
    return $result;
}
...
function dal_write_event($entity_id = "NULL", $target_role, $message, $type)
{
    ...
    $stmt = mysqli_prepare($connection, "INSERT INTO " . get_event_db_name() . ".event (target_id, target_role, message, type) VALUES ($entity_id,?,?,?)");
    if (!$stmt) {
        add_dal_error($connection, $db_errors);
        return false;
    }
